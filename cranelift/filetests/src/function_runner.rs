...
impl<'a> Trampoline<'a> {
    ...
    unsafe fn call_raw(
        ...
        match self.module.isa().triple().architecture {
            ...
            // NOTE: riscv32 is now handled by compiling to ELF and using the object runner,
            // not through this JIT trampoline path. This branch is unused but kept for
            // compatibility in case it's needed in the future.
