...
use cranelift_codegen::isa::{CallConv, lookup as isa_lookup};
...
fn test_structreturn_clif(
    ...
    let struct_ret_index = ctx.func.signature
    ...
    for (i, &val) in expected_values.iter().enumerate() {
        ...
        builder.ins().store(MemFlags::trusted(), const_val, ret_ptr, offset);
    }
    ...
    let compiled = ctx2.compile(module.isa(), &mut Default::default())
    ...
    println!("Saved JIT code ({} bytes) to /tmp/jit_code.bin", code_buffer.len());
    ...
    if buffer_native == expected_values {
        ...
        return Err(format!("Native function failed: expected {:?}, got {:?}", expected_values, buffer_native));
    }
    ...
    if buffer == expected_values {
        ...
        Err(format!("❌ FAILED on {}: Expected {:?}, got {:?}", isa_name, expected_values, buffer))
    }
}
...
fn main() {
    ...
    match test_native() {
        Ok(()) => {
            ...
            println!("\nTo examine assembly:");
            println!("  otool -tv target/release/test-structreturn | grep -A 20 native_structreturn");
