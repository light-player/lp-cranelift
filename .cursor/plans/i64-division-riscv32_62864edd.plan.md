...
2. For 64 iterations (bit positions 63 down to 0):
   ...
   - Shift `remainder` left by 1 (brings in next bit)
...
1. Check for special cases (divisor = 0, INT_MIN / -1 overflow)
2. Convert to unsigned (take absolute values, remember signs)
...
&Inst::DivI64U {
    ...
    // Implementation: see Step 3 for details
    // Uses shift_temp register
    
    // Shift quotient left by 1 (64-bit shift)
    // Uses same shift_temp register
    // ... (same 64-bit shift pattern for quotient) ...
...
### Step 6: Add Helper for Safe Signed Divisor (i64)
...
### Step 7: Implement Operand Tracking
...
// Similar for RemI64U and RemI64S
```
...
### Step 8: Implement Worst-Case Size Handling
...
### Step 9: Implement Display/Debug for New Instructions

File: `cranelift/codegen/src/isa/riscv32/inst/mod.rs`

Add pretty-printing support for the new instructions in the `Display` implementation:

```rust
Inst::DivI64U { dividend, divisor, quotient, rem, counter } => {
    write!(f, "div.i64u {}, {}, {} -> {}", 
           pretty_print_value_regs(dividend),
           pretty_print_value_regs(divisor),
           pretty_print_value_regs(quotient))
}
// Similar for DivI64S, RemI64U, RemI64S
```
