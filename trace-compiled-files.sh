...
cargo tree --package "$PACKAGE" --target "$TARGET" -e normal --prefix none | tee analysis-output/dep-tree.txt
echo ""
...
echo "(Full output in analysis-output/dep-tree-features.txt)"
cargo tree --package "$PACKAGE" --target "$TARGET" -e features --prefix none > analysis-output/dep-tree-features.txt
echo ""
...
cargo rustc --package "$PACKAGE" --target "$TARGET" --release -- --emit=dep-info -v 2>&1 | tee analysis-output/build-verbose.log | grep "Compiling" || true

# Also try with cargo build -v to see all rustc invocations
echo ""
echo "Analyzing .d files to find all source files..."
find target/$TARGET/release -name "*.d" -newer analysis-output 2>/dev/null | while read dfile; do
    echo "  From: $(basename $dfile)"
    cat "$dfile" | tr ' ' '\n' | grep "\.rs$" | head -5
done | tee analysis-output/source-files-from-d.txt | head -20
echo "(More in analysis-output/source-files-from-d.txt)"
echo ""
...
if [ -f "$BINARY" ]; then
    ...
    # Show section sizes
    echo "Section sizes:"
    size "$BINARY" | tee analysis-output/binary-size.txt
    echo ""
    ...
    if command -v nm &> /dev/null; then
        ...
        nm "$BINARY" 2>/dev/null | \
            ...
            sed 's/.* [TtRrDdBb] //' | \
            sed 's/_.*$//' | \
            sort | uniq -c | sort -rn | head -20 | tee analysis-output/symbol-prefixes.txt
        echo ""
    fi
fi
...
if command -v cargo-bloat &> /dev/null; then
    cargo bloat --release --target "$TARGET" --package "$PACKAGE" -n 30 | tee analysis-output/bloat-report.txt
    echo ""
    
    echo "Crate-level breakdown:"
    cargo bloat --release --target "$TARGET" --package "$PACKAGE" --crates | tee analysis-output/bloat-crates.txt
else
...
echo ""
echo "Output files created in analysis-output/:"
