...
Add validation method to `Riscv32Backend`:
...
**Integration point**: The validator should be called early in the compilation pipeline, before lowering. This happens in `cranelift/codegen/src/machinst/compile.rs` in the `compile()` function.
...
impl<'a> Validator<'a> {
    ...
    fn check_extension(&self, ext: supported::RiscvExtension) -> bool {
        match ext {
            ...
            supported::RiscvExtension::C => self.backend.isa_flags.has_c(),
...
/// Get the required extensions for a CLIF opcode
/// Returns a vector of extension names that must all be enabled
/// Empty vector means no extensions required (base ISA)
pub fn required_extensions(opcode: Opcode) -> Vec<&'static str> {
    ...
    match opcode {
        ...
        Opcode::Imul | Opcode::Sdiv | Opcode::Udiv | Opcode::Srem | Opcode::Urem => vec!["M"],

        // Atomic operations requiring A extension
        Opcode::AtomicRmw | Opcode::AtomicCas => vec!["A"],

        // Floating point requiring F/D extensions
        Opcode::Fadd | Opcode::Fsub | Opcode::Fmul | Opcode::Fdiv
        | Opcode::Fmin | Opcode::Fmax | Opcode::Fabs | Opcode::Fneg
        | Opcode::Sqrt | Opcode::Fma => {
            // These require F or D depending on the type
            // Type-specific check happens in validate_instruction_types
            vec!["F"] // Minimum F, but may require D for f64
        },

        // Bit manipulation requiring Zbb extension
        Opcode::Rotl | Opcode::Rotr | Opcode::Clz | Opcode::Ctz | Opcode::Popcnt => vec!["Zbb"],

        // Address generation requiring Zba extension
        Opcode::IaddImm if /* check if uses Zba pattern */ => vec!["Zba"],
...
pub fn type_required_extensions(ty: Type) -> Vec<&'static str> {
    match ty {
        Type::F32 => vec!["F"],
        Type::F64 => vec!["D", "F"], // D requires F
        Type::F16 => vec!["Zfh"], // Full support requires Zfh
        _ => vec![],
    }
}
