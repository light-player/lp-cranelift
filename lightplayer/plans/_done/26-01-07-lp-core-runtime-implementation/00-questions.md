...
6. **Shader Node Runtime** - Implement `nodes/shader/runtime.rs` and `nodes/shader/config.rs` (uses `GlslExecutable` from `lp-glsl` crate) (+ tests)
...
**Note**: `GlslExecutable` trait already exists in `lp-glsl` crate, so we can use that directly.
