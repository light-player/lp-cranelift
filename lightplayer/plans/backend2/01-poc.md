...
### 5. Tests (`tests/backend2/phase1.rs`)

#### Common Function Builder (Standin for Frontend)

```rust
/// Build the test functions (add and main) into a GlModule
/// This simulates what the frontend will do - build functions directly in the Module
fn build_test_functions<M: Module>(
    gl_module: &mut GlModule<M>,
) -> Result<(), GlslError> {
    ...
    build_simple_function(gl_module, "add", Linkage::Local, add_sig, |builder| {
    ...
    build_call_function(gl_module, "main", Linkage::Export, main_sig, "add", |builder| {
...
fn test_jit_function_call() {
    ...
    build_test_functions(&mut gl_module)?;
...
fn build_multiple_call_test<M: Module>(
    gl_module: &mut GlModule<M>,
) -> Result<(), GlslError> {
    ...
    build_simple_function(gl_module, "multiply", Linkage::Local, mult_sig, |builder| {
        let entry = builder.current_block().unwrap();
        let a = builder.block_params(entry)[0];
        let b = builder.block_params(entry)[1];
    ...
    build_simple_function(gl_module, "main", Linkage::Export, main_sig, |builder| {
        // Call multiply(2, 3)
        let two = builder.ins().iconst(types::I32, 2);
        let three = builder.ins().iconst(types::I32, 3);
        let mult_func = gl_module.get_func("multiply").unwrap();
        let mult_ref = gl_module.module_mut().declare_func_in_func(mult_func.func_id, builder.func);
        ...
        let add_func = gl_module.get_func("add").unwrap();
        let add_ref = gl_module.module_mut().declare_func_in_func(add_func.func_id, builder.func);
...
fn test_jit_multiple_calls() {
    let target = TargetSpec::host_jit()?;
    let mut gl_module = GlModule::new(target)?;
    build_multiple_call_test(&mut gl_module)?;
