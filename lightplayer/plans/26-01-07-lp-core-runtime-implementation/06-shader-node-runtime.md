...
1. Add `lp-glsl` dependency to `lp-core/Cargo.toml`:
   - `lp-glsl = { path = "../lp-glsl" }` (or workspace reference)
...
3. Create `nodes/shader/runtime.rs` with:
   ...
   - Implement `NodeLifecycle` trait:
     ...
     - `init()`: 
       ...
       - Compiles GLSL using `lp-glsl` compiler
...
- GLSL compilation works (using lp-glsl)
