...
pub fn load_sections(
    ...
    for symbol in obj.symbols() {
        if let Ok(name) = symbol.name() {
            if name == "__data_source_start" {
                ...
                debug!("Found __data_source_start symbol at 0x{:x}", symbol.address());
                break;
            }
        }
    }
    ...
    for section in obj.sections() {
        ...
        debug!("  Section '{}': LMA=0x{:x}, VMA=0x{:x}, size={}, kind={:?}", 
               section_name, section_addr, vma, data.len(), section_kind);
        ...
        if is_rom_address(vma) {
            ...
            if offset + data.len() > rom.len() {
                return Err(format!(
                    ...
                    section_name, offset, data.len(), rom.len()
                ));
            }
            debug!("    -> Copying {} bytes to code[0x{:x}..0x{:x}]", 
                   data.len(), offset, offset + data.len());
            ...
            if section_name == ".rodata" {
                ...
                debug!("Tracked .rodata end at 0x{:x} for .data LMA calculation", rodata_end.unwrap());
            }
            ...
            let rom_offset = if section_name == ".data" {
                ...
                data_source_start.map(|lma| lma as usize)
            ...
            if section_name == ".data" {
                debug!("    -> .data section LMA: 0x{:x} (from __data_source_start: {:?}, from .rodata end: {:?})", 
            ...
            if rom_offset + data.len() > rom.len() {
                return Err(format!(
                    ...
                    section_name, rom_offset, data.len(), rom.len()
                ));
            }
            debug!("    -> Copying {} bytes to ROM[0x{:x}..0x{:x}] (LMA) - will be copied to RAM[0x{:x}..0x{:x}] (VMA) by init code", 
                   data.len(), rom_offset, rom_offset + data.len(), 
                   ram_address_to_offset(vma), ram_address_to_offset(vma) + data.len());
            ...
            debug!("    -> Skipping section (VMA 0x{:x} not in ROM or RAM range)", vma);
        }
