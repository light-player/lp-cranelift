...
pub fn parse_elf(elf_data: &[u8]) -> Result<object::File, String> {
...
pub fn validate_elf(obj: &object::File) -> Result<(), String> {
    ...
    match obj.architecture() {
        ...
        arch => {
            return Err(format!("Unsupported architecture: {:?}. Expected RISC-V 32-bit", arch));
        }
    }
    ...
    match obj.endianness() {
        ...
        endian => {
            return Err(format!("Unsupported endianness: {:?}. Expected little-endian", endian));
        }
    }
