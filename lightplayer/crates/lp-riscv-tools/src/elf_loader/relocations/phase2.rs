...
pub fn apply_relocations_phase2(
    ...
    debug!("Applying {} GOT entry relocations first, then {} other relocations", 
           got_relocs.len(), other_relocs.len());
...
fn apply_single_relocation(
    ...
    let section_info = section_addrs.get(&reloc.section_name)
        .ok_or_else(|| format!(
        ...
        super::section::BufferSlice::Rom { offset } => {
            if *offset + reloc.offset as usize >= rom.len() {
                return Err(format!(
                    ...
                    *offset + reloc.offset as usize, reloc.section_name, rom.len()
                ));
            }
        ...
        super::section::BufferSlice::Ram { offset } => {
            if *offset + reloc.offset as usize >= ram.len() {
                return Err(format!(
                    ...
                    *offset + reloc.offset as usize, reloc.section_name, ram.len()
                ));
            }
    ...
    match reloc.r_type {
        1 => { // R_RISCV_32
            ...
            if got_tracker.has_entry(&reloc.symbol_name) {
                ...
                debug!("  Applied R_RISCV_32 at 0x{:x}: ✓ GOT entry initialized: '{}' = 0x{:x}", 
                       reloc.address, reloc.symbol_name, target_addr);
            } else {
                debug!("  Applied R_RISCV_32 at 0x{:x}: Wrote 0x{:x} to offset 0x{:x} for '{}'", 
        ...
        17 => { // R_RISCV_CALL_PLT
            ...
            handle_call_plt(&mut ctx, reloc)
                .map_err(|e| format!("Failed to apply R_RISCV_CALL_PLT at 0x{:x}: {}", reloc.address, e))?;
        }
        19 => { // R_RISCV_GOT_HI20
            let mut ctx = RelocationContext {
                buffer: buffer_slice,
            ...
            handle_got_hi20(&mut ctx, reloc)
                .map_err(|e| format!("Failed to apply R_RISCV_GOT_HI20 at 0x{:x}: {}", reloc.address, e))?;
        }
        20 => { // R_RISCV_PCREL_HI20
            ...
            handle_pcrel_hi20(&mut ctx, reloc)
                .map_err(|e| format!("Failed to apply R_RISCV_PCREL_HI20 at 0x{:x}: {}", reloc.address, e))?;
        }
        21 | 24 => { // R_RISCV_PCREL_LO12_I
            let mut ctx = RelocationContext {
                buffer: buffer_slice,
                pc,
                target_addr,
            ...
            handle_pcrel_lo12_i(&mut ctx, reloc)
                .map_err(|e| format!("Failed to apply R_RISCV_PCREL_LO12_I at 0x{:x}: {}", reloc.address, e))?;
        }
        _ => {
            return Err(format!(
