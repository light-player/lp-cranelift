    ...
    for section in obj.sections() {
        ...
        for (reloc_offset, reloc) in section.relocations() {
            ...
            let r_type = match reloc.flags() {
                ...
                _ => {
                    debug!("  Warning: Unsupported relocation format in section '{}' at offset 0x{:x}", 
            ...
            let load_addr = if section_info.vma >= 0x80000000 && section.kind() == object::SectionKind::Data {
                section_info.lma
            } else {
                section_info.vma
            };
        ...
        if !section_relocs.is_empty() {
            debug!("Section '{}' (VMA: 0x{:x}, LMA: 0x{:x}): {} relocations", 
                   section_name, section_info.vma, section_info.lma, section_relocs.len());
