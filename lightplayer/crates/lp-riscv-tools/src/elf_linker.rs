...
use object::{
    ...
    write::{
        Object as WriteObject, Relocation, SectionId, StandardSegment, Symbol, SymbolId,
        SymbolSection,
    },
};
...
/// Normalize section names for merging (e.g., .text.* -> .text, .rodata.* -> .rodata)
fn normalize_section_name(name: &str, kind: SectionKind) -> String {
    match kind {
        SectionKind::Text => {
...
pub fn link_static_library(
    ...
    for section in main_obj.sections() {
        ...
        let section_id = *section_name_map
    ...
    // 4. Copy main ELF symbols (keep original addresses)
    ...
    for member_data in archive_members {
        ...
        // Append sections to matching normalized sections
        for section in member_obj.sections() {
            if section.kind() == SectionKind::Metadata
                || section.data().ok().map(|d| d.is_empty()).unwrap_or(true)
            {
                continue;
            }
            ...
            // Get current section size (this is the offset where we'll place this data)
            let offset = *section_sizes.get(&normalized_name).unwrap_or(&0);
            // Store offset using original section name (for symbol lookup)
            ...
            // Update section size after appending
            *section_sizes.entry(normalized_name).or_insert(0) += data.len() as u64;
        }

        // Copy defined symbols from archive (adjust addresses by section offset)
        for symbol in member_obj.symbols() {
            ...
            // Adjust symbol address: add offset where we placed the section
            let symbol_value = match symbol.section() {
                object::SymbolSection::Section(section_index) => {
                    if let Ok(member_section) = member_obj.section_by_index(section_index) {
                        if let Ok(original_section_name) = member_section.name() {
                            // Use the offset we stored BEFORE appending
                            let offset = *member_section_offsets
                                .get(original_section_name)
                                .unwrap_or(&0);
                            symbol.address().wrapping_add(offset)
                        } else {
                            symbol.address()
                        }
                    } else {
            ...
            // Replace symbol (archive has defined version)
            let symbol_id = writer.add_symbol(Symbol {
                name: name.as_bytes().to_vec(),
                value: symbol_value,
                size: symbol.size(),
        ...
        for section in member_obj.sections() {
            ...
            // Get the offset where we placed this section's data
            let section_offset = *member_section_offsets.get(original_name).unwrap_or(&0);

            for (reloc_offset, reloc) in section.relocations() {
                // Adjust relocation offset: add the section offset where we placed the data
                let adjusted_offset = reloc_offset + section_offset;
...
/// Uses normalized section names to match merged sections.
        ...
        object::SymbolSection::Section(section_index) => {
            if let Ok(section) = obj.section_by_index(section_index) {
                if let Ok(original_section_name) = section.name() {
                    ...
                    let normalized_name =
                        normalize_section_name(original_section_name, section_kind);
...
fn convert_reloc_flags(
    flags: RelocationFlags,
) -> Result<object::write::RelocationFlags, LinkerError> {
    match flags {
...
mod tests {
    ...
    fn create_main_object() -> Vec<u8> {
        ...
        let ext_func_id = module
            .declare_function("external_func", Linkage::Import, &ext_sig)
            .unwrap();
    ...
    fn create_library_object() -> Vec<u8> {
        ...
        let ext_func_id = module
            .declare_function("external_func", Linkage::Export, &ext_sig)
            .unwrap();
    ...
    fn test_link_static_library() {
            ...
            if let Ok(name) = symbol.name() {
                if name == "external_func" {
                    ...
                    if section != object::SymbolSection::Undefined {
                        ...
                        println!(
                            "external_func: section={:?}, address=0x{:x}",
                            section, external_addr
                        );
        ...
        assert!(
            found_external,
            "external_func symbol should be found and defined in linked ELF"
        );
        ...
        let main_addr =
            crate::elf_loader::find_symbol_address(&obj, "main", 0).expect("main symbol not found");
        ...
        if main_addr as usize + 16 < load_info.code.len() {
            println!(
                "Code at main (first 16 bytes): {:02x?}",
                &load_info.code[main_addr as usize..main_addr as usize + 16]
            );
        }
    ...
    fn test_link_with_actual_builtins() {
        ...
        let builtins_lib = match find_builtins_library() {
            ...
            None => {
                println!(
                    "Skipping test: builtins library not found. Build it with: cargo build --target riscv32imac-unknown-none-elf --package lp-builtins"
                );
                return;
            }
        };
        ...
        assert!(
            ...
            "main symbol should have a section (not Undefined)"
        );
            ...
            if let Ok(name) = symbol.name() {
                if name == "__lp_fixed32_sqrt" {
                    ...
                    if section != object::SymbolSection::Undefined {
                        ...
                        assert_ne!(
                            sqrt_addr, 0,
                            "__lp_fixed32_sqrt symbol address should not be 0"
                        );
                        break;
                    }
                }
        ...
        let main_addr_from_loader =
            crate::elf_loader::find_symbol_address(&obj, "main", text_section_base)
