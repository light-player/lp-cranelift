...
impl GlSourceMap {
    ...
    pub fn to_source_location(&self, span: &GlSourceSpan) -> crate::frontend::src_loc::SourceLocation {
        if let Some(file) = self.get_file(span.file_id) {
            crate::frontend::src_loc::SourceLocation::with_file(
                span.start_line,
                span.start_column,
                String::from(file.source.display_name()),
            )
        } else {
            crate::frontend::src_loc::SourceLocation::new(
                span.start_line,
                span.start_column,
            )
        }
...
mod tests {
    ...
    fn test_to_source_location() {
        ...
        assert_eq!(source_loc.filename, Some(String::from("test.glsl")));
        ...
        assert_eq!(invalid_loc.filename, None);
    }
