...
fn test_matrix_multiplication_expected_result() {
    ...
    match (mat_a, mat_b, expected) {
        (GlslValue::Mat2x2(a), GlslValue::Mat2x2(b), GlslValue::Mat2x2(exp)) => {
            // Verify the expected result structure (m[col][row])
            // Column 0: [23.0, 34.0]
            assert_eq!(exp[0][0], 23.0); // col0_row0
            assert_eq!(exp[0][1], 34.0); // col0_row1
            // Column 1: [31.0, 46.0]
            assert_eq!(exp[1][0], 31.0); // col1_row0
            assert_eq!(exp[1][1], 46.0); // col1_row1

            // Verify input matrices are correct (m[col][row])
            // mat_a: Column 0: [1.0, 2.0], Column 1: [3.0, 4.0]
            assert_eq!(a[0][0], 1.0); // col0_row0
            assert_eq!(a[0][1], 2.0); // col0_row1
            assert_eq!(a[1][0], 3.0); // col1_row0
            assert_eq!(a[1][1], 4.0); // col1_row1

            // mat_b: Column 0: [5.0, 6.0], Column 1: [7.0, 8.0]
            assert_eq!(b[0][0], 5.0); // col0_row0
            assert_eq!(b[0][1], 6.0); // col0_row1
            assert_eq!(b[1][0], 7.0); // col1_row0
            assert_eq!(b[1][1], 8.0); // col1_row1
        }
