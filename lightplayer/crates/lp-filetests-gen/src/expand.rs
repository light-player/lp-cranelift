...
fn expand_single_specifier(specifier: &str, filetests_dir: &Path) -> Result<Vec<TestSpec>> {
    ...
    if specifier.ends_with(".gen") {
        ...
        return extract_spec_from_gen_file(specifier.strip_suffix(".gen").unwrap_or(specifier), filetests_dir);
    }
    ...
    bail!("Invalid specifier: {}. Expected format: vec/vec4/fn-equal, vec/vec4, or a directory path", specifier);
}
...
fn extract_spec_from_gen_file(file_path: &str, filetests_dir: &Path) -> Result<Vec<TestSpec>> {
    ...
    let rel_path = rel_path.ok_or_else(|| anyhow::anyhow!("Could not make path relative: {}", file_path))?;
    
    // Extract specifier from path like "vec/vec4/fn-equal.gen.glsl"
    // -> "vec/vec4/fn-equal"
    let path_str = rel_path.to_string_lossy();
    let specifier = path_str.strip_suffix(".gen.glsl")
                ...
                if ext == "glsl" {
                    ...
                    if let Some(stem) = path.file_stem() {
                        if stem.to_string_lossy().ends_with(".gen") {
                            ...
                            let rel_path = path.strip_prefix(filetests_dir)
                            ...
                            match parse_specifier(specifier) {
                                ...
                                Err(e) => {
                                    eprintln!("Warning: Failed to parse specifier from {}: {}", path.display(), e);
                                }
                            }
                        }
                    }
    ...
    match parts.len() {
        1 => {
            ...
            if parts[0] == "vec" {
                ...
                bail!("Invalid specifier: {}. Expected format: vec/vec4/fn-equal or vec/vec3", specifier);
            }
        }
        2 => {
            ...
            if first == "vec" {
                ...
                if let Ok((vec_type, dimension)) = parse_type_and_dimension(second) {
                    ...
                    bail!("Invalid dimension or type: {}. Expected vec2, vec3, vec4, ivec2, etc.", second);
                }
            } else {
                bail!("Invalid specifier: {}. Expected format: vec/vec4/fn-equal", specifier);
            }
        }
        3 => {
...
fn implemented_categories() -> Vec<&'static str> {
    vec!["fn-equal", "fn-greater-equal", "fn-greater-than", "fn-less-equal", "fn-less-than"]
...
fn generate_all_specs_for_type_and_dimension(vec_type: VecType, dimension: Dimension) -> Result<Vec<TestSpec>> {
    let mut specs = Vec::new();
    let categories = implemented_categories();
