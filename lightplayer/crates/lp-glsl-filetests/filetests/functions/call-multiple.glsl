...
float test_call_multiple_same_args() {
    ...
    return square(3.0) + square(3.0) + square(3.0); // 9 + 9 + 9 = 27
}
...
float test_call_multiple_different_args() {
    ...
    return power(2.0, 1.0) + power(2.0, 2.0) + power(2.0, 3.0); // 2 + 4 + 8 = 14
}
...
vec2 test_call_multiple_vector() {
    ...
    vec2 v1 = rotate90(vec2(1.0, 0.0));  // (0, 1)
    vec2 v2 = rotate90(vec2(0.0, 1.0));  // (-1, 0)
    vec2 v3 = rotate90(vec2(-1.0, 0.0)); // (0, -1)
...
float test_call_multiple_in_loop() {
    ...
    float scale_by_index(float base, int index) {
        return base * float(index + 1);
    }

    float total = 0.0;
    for (int i = 0; i < 3; i++) {
        total = total + scale_by_index(2.0, i);
    }
...
float test_call_multiple_nested() {
    ...
    // ((1+2) * (3+4)) + ((5+6) * (7+8))
    return multiply(add(1.0, 2.0), add(3.0, 4.0)) +
           multiply(add(5.0, 6.0), add(7.0, 8.0));
}

// run: test_call_multiple_nested() ~= 100.0
