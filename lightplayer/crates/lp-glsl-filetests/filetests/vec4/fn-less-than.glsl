...
bvec4 test_vec4_less_than_variables() {
    vec4 a = vec4(10.0, 15.0, 9.0, 7.0);
    vec4 b = vec4(12.0, 10.0, 8.0, 6.0);
    return lessThan(a, b);
}

// run: test_vec4_less_than_variables() == bvec4(true, false, false, false)

bvec4 test_vec4_less_than_expressions() {
    return lessThan(vec4(3.0, 7.0, 8.0, 4.0), vec4(5.0, 5.0, 6.0, 3.0));
}

// run: test_vec4_less_than_expressions() == bvec4(true, false, false, false)
...
// run: test_vec4_less_than_in_expression() == bvec4(true, false, false, false)
