...
vec4 test_vec4_from_scalar_positive() {
    ...
    return vec4(5.0);
    // Should be vec4(5.0, 5.0, 5.0, 5.0)
}
...
vec4 test_vec4_from_scalar_negative() {
    return vec4(-3.0);
    // Should be vec4(-3.0, -3.0, -3.0, -3.0)
}
...
vec4 test_vec4_from_scalar_variable() {
    float x = 42.0;
    return vec4(x);
    // Should be vec4(42.0, 42.0, 42.0, 42.0)
}
...
vec4 test_vec4_from_scalar_function_result() {
    ...
    // Should be vec4(7.0, 7.0, 7.0, 7.0)
}
...
vec4 test_vec4_from_scalar_in_assignment() {
    ...
    result = vec4(-8.0);
    return result;
    // Should be vec4(-8.0, -8.0, -8.0, -8.0)
}
...
vec4 test_vec4_from_scalar_large_value() {
    ...
    // Should be vec4(100000.0, 100000.0, 100000.0, 100000.0)
}
...
vec4 test_vec4_from_scalar_fractional() {
    return vec4(0.5);
    // Should be vec4(0.5, 0.5, 0.5, 0.5)
}
