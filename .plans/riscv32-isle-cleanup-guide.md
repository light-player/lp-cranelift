...
**Keep**: LrD, ScD, AmoswapD, etc. (64-bit atomics work on RV32 for doubleword operations)
...
### Rules Using `fits_in_32`

Many lowering rules check `fits_in_32` and use `*w` instructions. For RV32:
- Remove or modify any rule that uses `fits_in_32` to select `*w` instructions
- These rules are trying to optimize for 32-bit values on 64-bit arch - not needed on RV32

**Search for and remove/modify:**
- Rules matching `(fits_in_32 ...)`  that call `*w` instructions
- Rules for `I64` type that use `*w` variants

### Atomic Operations

Around lines ~2537-2603, there are rules for atomic RMW operations on I32:
...
**For RV32, change to use doubleword atomics:**
```isle
(rule
  (get_atomic_rmw_op $I32 (AtomicRmwOp.Add))
  (AtomicOP.AmoaddD))  ;; Use D (doubleword) variant on RV32
```

**Rationale**: On RV32, a 32-bit value is a full word, so we use the "doubleword" (64-bit) atomic ops which work on 32-bit registers.
...
1. Start with `inst.isle`:
   ...
   - Remove helper functions (rv_*w) one section at a time
...
**Atomic:**
- `amo*w` - All atomic word operations (use `amo*d` on RV32 instead)
...
- Keep Zba, Zbb, Zbc, Zbs extensions (just remove *w variants)
- Keep floating-point and vector extensions (no *w variants there)
- Atomic operations: use `*d` (doubleword = 64-bit) variants on RV32 for 32-bit values
