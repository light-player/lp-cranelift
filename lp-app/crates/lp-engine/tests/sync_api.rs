...
fn test_sync_api_get_changes() {
    ...
    let response = runtime
        .get_changes(lp_model::FrameId::default(), &ApiNodeSpecifier::All)
        .unwrap();
...
fn test_sync_api_no_changes() {
    ...
    let response = runtime
        .get_changes(current_frame, &ApiNodeSpecifier::All)
        .unwrap();

    match response {
        ProjectResponse::GetChanges { node_changes, .. } => {
...
fn test_sync_api_by_handles() {
    ...
    fs.write_file_mut("/src/test1.texture/node.json", texture_json.as_bytes())
        .unwrap();
    fs.write_file_mut("/src/test2.texture/node.json", texture_json.as_bytes())
        .unwrap();
    ...
    match response {
        ProjectResponse::GetChanges { node_details, .. } => {
