//! JIT compiler for the toy language using Cranelift.
...
impl JIT {
    ...
    pub fn new() -> Self {
        ...
        // Note: JIT requires is_pic=false
    ...
    /// Create a new JIT instance with a custom ISA (for no_std).
    #[cfg(not(feature = "std"))]
    pub fn new() -> Self {
        panic!("JIT::new() requires std feature. Use JIT::with_isa() in no_std mode.");
    }

    /// Create a new JIT instance with a custom ISA and builder.
    #[cfg(not(feature = "std"))]
    pub fn with_isa(_isa: OwnedTargetIsa, builder: JITBuilder) -> Self {
        let module = JITModule::new(builder);
        Self {
            builder_context: FunctionBuilderContext::new(),
            ctx: module.make_context(),
    ...
    /// Compile a string in the toy language and return the CLIF IR as a string.
    pub fn compile_to_clif(&mut self, input: &str) -> Result<String, String> {
        // Clear the context first to ensure clean state
        self.ctx.clear();
        ...
        #[cfg(not(feature = "std"))]
        {
            ...
            write!(&mut output, "{}", self.ctx.func).map_err(|e| e.to_string())?;
            Ok(output)
        }
    }
