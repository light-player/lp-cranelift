use core::mem;
use lp_toy_lang::jit::JIT;

fn main() -> Result<(), String> {
    // Check if we should output CLIF instead of running
    let args: Vec<String> = std::env::args().collect();
    if args.len() > 1 && args[1] == "--clif" {
        // Output CLIF for test cases
        let mut jit = JIT::default();

        println!("=== max function ===");
        println!("{}", jit.compile_to_clif(MAX_CODE)?);
        println!("\n=== sum function ===");
        println!("{}", jit.compile_to_clif(SUM_CODE)?);
    ...
    println!("max(42, 17) = {}", run_max(&mut jit, 42, 17)?);
    println!("sum(10) = {}", run_sum(&mut jit, 10)?);
    run_hello(&mut jit)?;
    Ok(())
}

fn run_foo(jit: &mut JIT) -> Result<isize, String> {
    unsafe { run_code(jit, FOO_CODE, (1, 0)) }
}

fn run_recursive_fib_code(jit: &mut JIT, input: isize) -> Result<isize, String> {
    unsafe { run_code(jit, RECURSIVE_FIB_CODE, input) }
}

fn run_iterative_fib_code(jit: &mut JIT, input: isize) -> Result<isize, String> {
    unsafe { run_code(jit, ITERATIVE_FIB_CODE, input) }
}

fn run_max(jit: &mut JIT, a: isize, b: isize) -> Result<isize, String> {
    unsafe { run_code(jit, MAX_CODE, (a, b)) }
}

fn run_sum(jit: &mut JIT, n: isize) -> Result<isize, String> {
    unsafe { run_code(jit, SUM_CODE, n) }
}

fn run_hello(jit: &mut JIT) -> Result<isize, String> {
...
/// Test case: max function (if-else)
const MAX_CODE: &str = r#"
fn max(a, b) -> (result) {
    if a > b {
        result = a
    } else {
...
/// Test case: sum function (while loop)
